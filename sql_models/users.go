...
var (
	userAllColumns            = []string{"user_id", "username", "email", "user_password", "created_at", "active"}
	userColumnsWithoutDefault = []string{"username", "email", "user_password", "created_at"}
	userColumnsWithDefault    = []string{"user_id", "active"}
...
func (o *User) CommentVotes(mods ...qm.QueryMod) commentVoteQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"comment_votes\".\"user_id\"=?", o.UserID),
	)
...
func (o *User) Comments(mods ...qm.QueryMod) commentQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"comments\".\"user_id\"=?", o.UserID),
	)
...
func (o *User) PostVotes(mods ...qm.QueryMod) postVoteQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"post_votes\".\"user_id\"=?", o.UserID),
	)

	query := PostVotes(queryMods...)
	queries.SetFrom(query.Query, "\"post_votes\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"post_votes\".*"})
	}
...
func (o *User) Posts(mods ...qm.QueryMod) postQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"posts\".\"user_id\"=?", o.UserID),
	)
...
func (userL) LoadCommentVotes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`comment_votes.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPostVotes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`post_votes`),
		qm.WhereIn(`post_votes.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddCommentVotes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CommentVote) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UserID, rel.CommentID, rel.UserID}
...
func (o *User) AddComments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Comment) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UserID, rel.CommentID}
...
// Sets related.R.User appropriately.
func (o *User) AddPostVotes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PostVote) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.UserID
			...
			values := []interface{}{o.UserID, rel.PostID, rel.UserID}
...
func (o *User) AddPosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Post) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.UserID, rel.PostID}
